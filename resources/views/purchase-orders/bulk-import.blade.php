<x-app-layout title="Import Massal Purchase Order">
    <x-container>
        <x-page-header 
            title="Import Massal Purchase Order" 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2">
                <x-card title="Upload File Import" subtitle="Pilih file yang berisi daftar bahan baku.">
                    <form action="{{ route('purchase-orders.bulk-import.process') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        ...
                        <x-datepicker label="Tanggal Purchase Order" name="po_date" required value="{{ old('po_date', now()->format('Y-m-d')) }}" />

                        <x-form-textarea label="Catatan Tambahan (Opsional)" name="notes" placeholder="Contoh: Import dari laporan mingguan..." value="{{ old('notes') }}" />
            ...
            <div class="space-y-6">
                <x-card title="Petunjuk Format" :padding="true">
                    <div class="space-y-4">
                            ...
                            <div class="flex gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-[12px] text-amber-800 leading-relaxed">
                                        Jika bahan makanan belum terdaftar di sistem, sistem akan <strong>otomatis menambahkannya</strong> sebagai bahan baku baru ke katalog dapur.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <ul class="space-y-2">
                                @php 
                                    $cols = ['No', 'Uraian Jenis Bahan Makanan', 'Kuantitas', 'Satuan', 'Harga Satuan', 'Jumlah', 'Keterangan'];
                                @endphp
                                @foreach($cols as $i => $col)
                                <li class="flex items-start gap-2 text-[13px] text-slate-600">
                                    <span class="w-5 h-5 rounded-full bg-slate-100 text-slate-400 text-[10px] font-black flex items-center justify-center shrink-0 mt-0.5">{{ $i+1 }}</span>
                                    <span>{{ $col }}</span>
                                </li>
                                @endforeach
                            </ul>
                ...
                <x-btn variant="outline" class="w-full py-4! rounded-2xl!" href="{{ route('purchase-orders.download-template') }}">
