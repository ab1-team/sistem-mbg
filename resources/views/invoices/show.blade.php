<x-app-layout title="Detail Penagihan">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <x-table>
                    ...
                    @foreach($invoice->items as $item)
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            <x-table-td>
                                <p class="font-bold text-slate-900 tracking-tight leading-none mb-1">{{ $item->material->name }}</p>
                                <p class="font-mono text-[10px] text-slate-400 font-bold uppercase tracking-tighter">{{ $item->poItem->unit }}</p>
                            </x-table-td>
                    ...
                    <x-slot name="tfoot">
                        <tr class="bg-slate-50/50 border-t border-slate-100">
                            <td colspan="3" class="px-6 py-4 text-right text-[11px] font-bold text-slate-400 uppercase tracking-widest">Total Bayar</td>
                            <td class="px-6 py-4 text-right text-[18px] font-bold text-slate-900 tracking-tight">
        ...
        <div class="lg:col-span-1 space-y-6">
            <x-card title="Informasi Supplier" subtitle="Pihak yang harus dibayarkan tagihannya.">
                <div class="space-y-4">
                    <div>
                        <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">Nama Supplier</p>
                        <p class="text-[14px] font-bold text-slate-900">{{ $invoice->supplier->name }}</p>
                    </div>
                    <div>
                        <p class="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1">Dapur Pemesan</p>
                        <p class="text-[14px] font-bold text-slate-900">{{ $invoice->dapur->name }}</p>
                    </div>
                </div>
            </x-card>
            ...
            @if($invoice->status === 'dibayar')
                <x-card title="Bukti Pembayaran" subtitle="Dokumen verifikasi transaksi finansial.">
                    ...
                    <p class="mt-4 text-[11px] text-center text-slate-400 font-bold uppercase tracking-tighter">Dibayar pada {{ $invoice->paid_at->translatedFormat('d M Y, H:i') }}</p>
                </x-card>
            @endif
        </div>
    </div>
    ...
    <x-dialog name="modal-pay" title="Catat Pembayaran">
        <form action="{{ route('invoices.pay', $invoice) }}" method="POST" enctype="experimental" enctype="multipart/form-data">
            @csrf
            <div class="mb-6">
                <label class="block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-2">Unggah Bukti Transfer / Kwitansi</label>
                <input type="file" name="payment_proof" required 
                       class="w-full rounded-2xl border-slate-200 text-[14px] focus:ring-green-500 focus:border-green-500">
                <p class="mt-2 text-[11px] text-slate-400 italic font-medium leading-relaxed">
                    Unggah bukti pembayaran fisik (foto/scan) untuk catatan audit finansial.
                </p>
            </div>
