<x-app-layout title="Detail Penagihan">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            <x-card :padding="false" title="Rincian Tagihan Aktual"
                subtitle="Dihitung berdasarkan kuantitas barang yang benar-benar diterima (GR).">
                <x-table>
                    <x-slot name="thead">
                    ...
                    <x-slot name="tfoot">
                        <tr class="bg-slate-50/50 border-t border-slate-100">
                            <td colspan="3"
                                class="px-6 py-4 text-right text-[11px] font-medium text-slate-400 uppercase tracking-widest">
                                Total Bayar</td>
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            @if ($invoice->status === 'dibayar')
                <x-card title="Bukti Pembayaran" subtitle="Dokumen verifikasi transaksi finansial.">
                    <img src="{{ Storage::url($invoice->payment_proof) }}"
                        class="rounded-2xl w-full border border-slate-100 shadow-sm" alt="Bukti Transfer">
                    <p class="mt-4 text-[11px] text-center text-slate-400 font-medium uppercase tracking-tighter">
                        Dibayar pada {{ $invoice->paid_at->translatedFormat('d M Y, H:i') }}</p>
                </x-card>
            @endif
        </div>
    </div>
    ...
    <x-dialog name="modal-pay" title="Catat Pembayaran">
        <form action="{{ route('finance.invoices.pay', $invoice) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mb-6">
                <x-form-file name="payment_proof" label="Unggah Bukti Transfer / Kwitansi" required
