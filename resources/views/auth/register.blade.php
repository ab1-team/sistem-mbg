<x-guest-layout title="Daftar Akun Baru">
    ...
    <div class="mb-8 text-center animate-reveal" style="animation-delay: 0.1s">
        <h3 class="text-2xl font-bold text-slate-900 tracking-tight leading-none mb-3">
            Daftar <span class="text-emerald-700 font-extrabold">Akun Baru</span>
        </h3>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-6 animate-reveal" style="animation-delay: 0.2s">
        ...
        <!-- Name -->
        <div class="space-y-3">
            <x-form-input id="name" name="name" label="Nama Lengkap" type="text" :value="old('name')" required
        ...
        <div class="space-y-3">
            <x-form-input id="email" name="email" label="Alamat Email" type="email" :value="old('email')" required
                autocomplete="username" placeholder="john@yayasan-mbg.id">
                <x-slot name="icon">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path
        ...
        <!-- Confirm Password -->
        <div class="space-y-3">
            <x-form-input id="password_confirmation" name="password_confirmation" label="Konfirmasi Kata Sandi"
        ...
        <div class="text-center">
            <p class="text-[12px] text-slate-500 font-medium">
                Sudah memiliki akun?
                <a href="{{ route('login') }}"
                    class="text-emerald-700 hover:text-emerald-600 transition-all font-semibold underline underline-offset-4 decoration-emerald-700/20">Masuk
                    sekarang</a>
            </p>
        </div>
    </form>
