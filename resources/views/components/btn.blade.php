...
@php
    $base =
        'inline-flex items-center justify-center gap-2 whitespace-nowrap font-semibold rounded-2xl border transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 active:scale-95 duration-300 disabled:opacity-50 disabled:cursor-not-allowed';

    $variants = [
        'primary' =>
            'bg-brand-primary text-white border-transparent hover:bg-brand-accent focus:ring-brand-primary shadow-sm',
    ...
    <button type="{{ $type }}"
        ...
        @if ($loading)
            ...
            <span class="items-center gap-2" wire:loading.flex
                @if ($loadingTarget) wire:target="{{ $loadingTarget }}" @endif style="display: none;">
