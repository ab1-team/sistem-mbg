<div class="space-y-8 pb-32">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <x-card title="Informasi Dasar" subtitle="Tentukan target dapur dan periode perencanaan.">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div class="md:col-span-2">
                        <x-form-input label="Judul Perencanaan" wire:model="title" id="title" name="title"
                            placeholder="Contoh: Rencana Januari 2024 - Dapur A" required />
                    </div>
                    @if (count($dapurs) > 1)
                        <div>
                            <x-form-searchable-select label="Unit Dapur" name="dapur_id" wire:model.live="dapur_id"
                                :selected="$dapur_id" :options="$dapurs->map(fn($d) => ['value' => (string) $d->id, 'label' => $d->name])" placeholder="Pilih Dapur" required />
                        </div>
                    @endif

                    <div class="{{ count($dapurs) > 1 ? '' : 'md:col-span-2' }}">
                        @if (count($dapurs) <= 1)
                            <input type="hidden" wire:model="dapur_id">
                        @endif
                        <x-form-searchable-select label="Periode" name="period_id" wire:model.live="period_id"
                            :selected="$period_id" :options="$periods->map(
                                fn($p) => [
                                    'value' => (string) $p->id,
                                    'label' =>
                                        $p->name .
                                        ' (' .
                                        $p->start_date->format('d M') .
                                        ' - ' .
                                        $p->end_date->format('d M') .
                                        ')',
                                ],
        ...
        <div class="lg:col-span-1">
            <div
                class="bg-emerald-900 rounded-[24px] p-6 text-white relative overflow-hidden h-full flex flex-col justify-center">
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-[12px] text-emerald-100 opacity-80 leading-relaxed mb-4">Pilih periode terlebih
                        dahulu untuk memunculkan daftar tanggal penjadwalan secara otomatis.</p>
        ...
        <div class="space-y-4">
            <div class="flex items-center justify-between px-2">
                ...
                <div class="flex items-center gap-3">
                    @if ($dapur_id)
                        @php
                            $d = \App\Models\Dapur::find($dapur_id);
                        @endphp
                        @if ($d)
                            <button type="button" wire:click="syncPortions"
                                class="px-3 py-1 bg-green-50 text-green-700 text-[11px] font-bold rounded-lg border border-green-200 hover:bg-green-100 transition-colors flex items-center gap-2">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                        d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                                </svg>
                                Isi Otomatis: {{ $d->capacity_portions }} Porsi
                            </button>
                        @endif
                    @endif
                    <span
                        class="px-3 py-1 bg-slate-100 text-slate-500 text-[11px] font-bold rounded-lg">{{ count($schedules) }}
                        Hari</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                @foreach ($schedules as $date => $day)
                    <div
                        class="bg-white rounded-[24px] border border-slate-100 shadow-sm p-5 hover:border-green-200 transition-colors">
                        <div class="flex items-center justify-between mb-4 pb-3 border-b border-slate-50">
                            <span
                                class="text-[14px] font-black text-slate-900 tracking-tight">{{ $day['display'] }}</span>
                            <span
                                class="text-[10px] font-bold text-slate-300 uppercase tracking-widest font-mono">{{ $date }}</span>
                        </div>
                            ...
                            @foreach ($day['meals'] as $mIdx => $meal)
                                <div class="space-y-1.5 p-3 rounded-2xl bg-slate-50/50">
                                    ...
                                    <div class="flex gap-2">
                                        <div class="flex-1 min-w-0 relative" x-data="{
                                            ...
                                            <div @click="open = !open"
                                                ...
                                                <template x-if="!selectedIds || selectedIds.length === 0">
                                                    <span class="text-[12px] text-slate-400 px-2 py-1 font-bold">Pilih
                                                        Menu...</span>
                                                </template>

                                                <template x-for="id in (selectedIds || [])" :key="id">
                                                    <span
                                                        class="inline-flex items-center gap-1.5 px-2 py-0.5 bg-slate-100 text-slate-700 text-[10px] font-black rounded-lg border border-slate-200 group-hover:bg-green-50 group-hover:text-green-700 group-hover:border-green-100 transition-colors">
                                                ...
                                                <div class="ml-auto pr-1 flex items-center">
                                                    <svg class="w-3 h-3 text-slate-300 group-hover:text-green-500 transition-colors"
                                                ...
                                                <div class="space-y-0.5">
                                                    @foreach ($menuItems as $item)
                                                        <label
                                                            ...
                                                            <input type="checkbox"
                                                                :checked="(selectedIds || []).includes(parseInt(
                                                                    '{{ $item->id }}'))"
                                                                @change="toggle('{{ $item->id }}')"
                                                                class="w-4 h-4 rounded-lg border-slate-300 text-green-600 focus:ring-green-500/20 transition-all">
                                                            <span
                                                                class="text-[12px] font-black text-slate-700 leading-none">{{ $item->name }}</span>
                                                        </label>
                                                    @endforeach
                                                </div>
                                            </div>
                                        ...
                                        <div class="w-16">
                                            <input type="number"
                                                wire:model="schedules.{{ $date }}.meals.{{ $mIdx }}.portions"
                                                placeholder="Qty"
                                    ...
                                    @error('schedules.' . $date . '.meals.' . $mIdx . '.menu_item_ids')
                                        <p class="text-[9px] text-red-500 font-bold mt-1">Minimal satu menu harus
                                            dipilih
                                            jika porsi diisi</p>
                                    @enderror
                                    @error('schedules.' . $date . '.meals.' . $mIdx . '.portions')
                                        <p class="text-[9px] text-red-500 font-bold mt-1">Porsi tidak boleh kosong</p>
                                    @enderror
                                </div>
                            @endforeach
                        </div>
    ...
    @else
        <div class="py-20 flex flex-col items-center text-center">
            ...
            <p class="text-[12px] text-slate-400 mt-1 max-w-[300px]">Jadwal akan digenerate otomatis berdasarkan
                rentang tanggal periode tersebut.</p>
        </div>
    @endif
    ...
    <div
        ...
        <x-container class="!space-y-0 !pb-0 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto">
                <x-btn href="{{ route('menu-periods.index') }}" variant="secondary"
                    class="flex-1 sm:flex-none">Batal</x-btn>
                <x-btn type="button" wire:click="save" loading="true" loading-target="save"
                    loading-text="Menyimpan..." class="flex-1 sm:flex-none shadow-lg shadow-green-900/20">
                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" stroke-width="3"
                        viewBox="0 0 24 24">
