<div>
        ...
        <x-table>
            ...
            @forelse($periods as $period)
                <tr class="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <x-table-td>
                        <span class="font-black text-slate-900 group-hover:text-green-700 transition-colors uppercase tracking-tight">{{ $period->name }}</span>
                    </x-table-td>
                    <x-table-td>
                        <span class="text-slate-400 font-medium text-[11px]">{{ $period->start_date->format('d M') }} — {{ $period->end_date->format('d M Y') }}</span>
                    </x-table-td>
                    <x-table-td>
                    ...
                    <x-table-td>
                        @if($period->closedBy)
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-[9px] font-bold text-slate-500 border border-slate-200">
                                    {{ substr($period->closedBy->name, 0, 2) }}
                                </div>
                                <span class="text-slate-500 font-medium text-[11px]">{{ $period->closedBy->name }}</span>
                            </div>
                        @else
                    ...
                    <x-table-td class="text-right py-3 px-4">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            @elseif($period->isClosed())
                                <x-btn wire:click="reopenPeriod({{ $period->id }})" variant="secondary" class="py-1.5! px-3! text-[11px]!">Buka Kembali</x-btn>
                            ...
                            @else
                                <span class="text-slate-400 font-bold text-[10px] uppercase border border-slate-100 px-2 py-1 rounded-lg bg-slate-50">FINAL & TERKUNCI</span>
                            @endif
                        </div>
                    </x-table-td>
                </tr>
    ...
    <div class="bg-amber-50 border border-amber-100 rounded-[24px] p-6">
        ...
        <ul class="text-amber-800 text-[12px] space-y-2 list-disc list-inside leading-relaxed opacity-90">
            ...
            <li><strong>LOCKED:</strong> Laba bersih dikalkulasi dan dividen otomatis didistribusikan ke dompet investor. <em>(Tidak dapat dibuka kembali)</em></li>
        </ul>
    </div>
</div>
