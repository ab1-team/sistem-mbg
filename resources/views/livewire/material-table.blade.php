    ...
    <x-smart-table-actions>
        <div class="flex flex-wrap items-center gap-3">
            <x-dialog title="Import Bahan Baku" name="import-modal">
                ...
                <form action="{{ route('materials.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div class="p-4 border-2 border-dashed border-slate-100 rounded-2xl bg-slate-50/50 flex flex-col items-center gap-3 text-center">
                        ...
                        <input type="file" name="file" class="text-[12px] text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[11px] file:font-bold file:bg-green-50 file:text-green-700 hover:file:bg-green-100 transition-all" required>
                    </div>
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-xl p-3 flex gap-3">
                        ...
                        <div class="text-[11px] text-blue-700 leading-relaxed">
                            ...
                            <a href="{{ route('materials.download-template') }}" class="font-bold underline hover:text-blue-800 transition-colors">Unduh Template CSV</a>
                        </div>
                    </div>

                    <div class="flex justify-end gap-2 pt-2">
                        <x-btn type="button" variant="secondary" x-on:click="$dispatch('close-modal', 'import-modal')">Batal</x-btn>
        ...
        <x-table>
            ...
            @forelse($materials as $material)
                <tr class="hover:bg-slate-50/50 transition-colors group">
                    <x-table-td>
                        <span class="font-mono text-[12px] font-bold text-slate-400 tracking-tighter">{{ $material->code }}</span>
                    </x-table-td>
                    <x-table-td>
                        <span class="font-bold text-slate-900 tracking-tight">{{ $material->name }}</span>
                    </x-table-td>
                    <x-table-td>
                        <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-600 uppercase">{{ $material->category }}</span>
                    </x-table-td>
                    <x-table-td class="text-center font-medium text-slate-500">
                        {{ $material->min_stock_threshold }} <span class="text-[10px] uppercase">{{ $material->unit }}</span>
                    </x-table-td>
                    <x-table-td class="text-right py-3 px-4">
                        <div class="flex items-center justify-end gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-all duration-200">
                            <x-btn href="{{ route('materials.edit', $material) }}" variant="secondary" class="py-1.5! px-3! text-[11px]!">Edit</x-btn>
            ...
            @empty
                <tr>
                    <td colspan="5">
                        <x-empty-state title="Belum ada bahan baku" subtitle="Impor dari Excel atau tambah secara manual." />
                    </td>
                </tr>
            @endforelse
        </x-table>
