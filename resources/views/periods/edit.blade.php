<x-app-layout title="Edit Periode">
    ...
    <x-card class="max-w-2xl {{ $period->status != 'open' ? 'opacity-80 grayscale-[0.1]' : '' }}">
        <form action="{{ route('periods.update', $period) }}" method="POST" class="space-y-6">
            ...
            <div class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                <div class="flex items-center gap-3">
                    <div
                        class="w-11 h-11 rounded-xl bg-green-50 text-green-700 flex items-center justify-center font-bold text-[12px] border border-green-100 uppercase">
                        {{ $period->code }}
                    </div>
                    <div>
                        <h3 class="text-[13px] font-bold text-slate-900 uppercase tracking-tight">{{ $period->name }}
                        </h3>
                        <div class="flex items-center gap-1.5 mt-0.5">
                            <div
                                class="w-1.5 h-1.5 rounded-full {{ $period->status == 'open' ? 'bg-emerald-500' : 'bg-slate-400' }}">
                            </div>
            ...
            <div class="p-6 bg-slate-50 border border-slate-100 rounded-3xl">
                ...
                <div class="grid grid-cols-2 gap-6 text-[12px] font-medium">
                    ...
                    <div class="p-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
                        ...
                        <p class="text-slate-900 font-bold">{{ $period->closedBy ? $period->closedBy->name : 'N/A' }}
                        </p>
                    </div>
                </div>
            </div>
